...
void Room::setChest(bool hasChest) { chest = hasChest; }
...
void Room::describe() const {
  ...
  if (puzzle && !puzzle->getIsSolved()) {
    std::cout << "The room feels mysterious and foreboding. To proceed, you "
                 "must solve the following puzzle:\n"
...
std::shared_ptr<Item> Room::findItem(const std::string &itemName) const {
  ...
  for (const auto &item : items) {
    ...
    std::transform(normalizedItemName.begin(), normalizedItemName.end(),
                   normalizedItemName.begin(), ::tolower);
...
void Room::setNPC(std::shared_ptr<NPC> npc) { this->npc = npc; }

// Get the NPC in the room
std::shared_ptr<NPC> Room::getNPC() const { return npc; }

// Set the puzzle in the room
void Room::setPuzzle(std::shared_ptr<Puzzle> puzzle) { this->puzzle = puzzle; }

// Get the puzzle in the room
std::shared_ptr<Puzzle> Room::getPuzzle() const { return puzzle; }

// Check if the puzzle in the room is solved
bool Room::isPuzzleSolved() const { return puzzle && puzzle->getIsSolved(); }

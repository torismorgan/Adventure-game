...
void Player::move(const std::string &direction) {
  ...
  if (room) {
    if (currentRoom
            ->isPuzzleSolved()) { // Ensure the puzzle is solved before moving
      currentRoom = room;
...
std::shared_ptr<Item>
Player::findItemInInventory(const std::string &itemName) const {
  ...
  for (const auto &item : inventory) {
    ...
    std::transform(normalizedStoredName.begin(), normalizedStoredName.end(),
                   normalizedStoredName.begin(), ::tolower);
...
void Player::lightTorch() {
  hasTorchLit = true;
  std::cout
      << "The torch is now lit. It flickers, illuminating the darkness.\n";
}

// Check if the torch is currently lit
bool Player::isTorchLit() const { return hasTorchLit; }
